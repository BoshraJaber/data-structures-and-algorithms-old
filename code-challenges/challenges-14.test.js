...
const screenForNames = (arr) => {
  let newArr = [];
  let regex = /^(Mr\.|Dr\.|Er\.|Ms\.|Mrs\.)\s?[a-z|A-Z]+$/;
  // let regex2 = /^[a-zA-Z\s]*$/;
  arr.forEach(str => {
    if (regex.test(str)) {
      newArr.push(str)
...
const toTitleCase = (arr) => {
  let capitalArr = [];
  arr.forEach(str => {
    capitalArr.push(str.charAt(0).toUpperCase() + str.slice(1))
  })
  return capitalArr;
};
...
let biggerThanLuke = (arr) => {
  let luckMass = Number(starWarsData[0].mass);
  let resultArr = [];
  arr.forEach(charactor => {
    if (Number(charactor.mass) > luckMass) {
      resultArr.push(charactor.name)
    }
    //  console.log(charactor.mass)
