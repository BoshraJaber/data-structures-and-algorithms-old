...
const addOne = (arr) => {
  let arrayOfNumbers = arr.map( function(n,i) {
    return n +1;
  });
...
const addQuestion = (arr) => {
  let arrayOfStrings = arr.map(function(n,i){
    return n+'?';
  });
...
const forEachTwoToThe = (arr) => {
  let arrayOfPower=[];
  arr.forEach(element => {
    arrayOfPower.push(Math.pow(2, element))
  });
  return arrayOfPower;
};
...
const mapTwoToThe = (arr) => {
  let arrayOfPower = arr.map(function(n,i){
    return Math.pow(2, n);
  })
  return arrayOfPower;
};
